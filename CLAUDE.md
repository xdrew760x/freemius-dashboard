...
- **`index.php`** — markup only: header (with product dropdown), tabs, toolbar, table, paginator, confirm modal, and user-detail drawer. Tailwind via CDN. Ends with `<script src="dashboard.js?v=..."></script>`.
- **`dashboard.js`** — all dashboard JS. Four tabs (users / licenses / subscriptions / installs) share one table render path. State lives in globals (`currentTab`, `currentOffset`, `currentProductId`); per-tab config lives in the `filters` / `headers` / `sortKeys` maps at the top. Every `fetch('api.php?action=...')` appends `&product_id=${currentProductId}`. The header dropdown is populated from `?action=list_products`; selection persists in `localStorage['freemius.productId']`.
